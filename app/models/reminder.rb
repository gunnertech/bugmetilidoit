  ...
  def send_notices
    ...
    if ENV['BLOWERIO_URL']
      ...
      @client.account.sms.messages.create(
        :from => '+12038197645',
        :to => self.user.mobile,
        :body => "#{self.task.title}. Did it? Click here: #{complete_assigned_task_url(self.assigned_task, auth_token: self.user.authentication_token, assigned_task: {action: "complete"})}"
      ) rescue nil
