class Reminder < ActiveRecord::Base
  ...
  def send_notices
    ...
    ReminderMailer.reminder_email(self,self.user).deliver if networks.include?(Network.find_by_name('Email'))
    
    if networks.include?(Network.find_by_name('SMS'))
      if ENV['BLOWERIO_URL']
    ...
    if networks.include?(Network.find_by_name('Twitter'))
    end
